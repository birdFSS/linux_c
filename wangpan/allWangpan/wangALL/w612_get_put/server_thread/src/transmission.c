...
int transmiss(int tranFd, MYSQL* conn,linkMsg_t *plmsg, UserState_t* pUState){
     ...
     * 一个是文件表正常下载，这里先正常下载。
     */
    char TypeMd5SizeOffsetStr[300] = {0};
    size_t offset = 0;
    //正常下载只有md5，文件类型，自己手动加上offset 0
    //plmsg->buf里是文件名
    ret = getFileInfo(conn, plmsg->buf, pUState->currentDirId, TypeMd5SizeOffsetStr);
    if(-1 == ret)
    {
    ...
    char offsetStr[4]=" 0";
    strcat(TypeMd5SizeOffsetStr, offsetStr);


    //将数据发送回去
    bzero(plmsg->buf, sizeof(plmsg->buf));
    strcpy(plmsg->buf,TypeMd5SizeOffsetStr);
    plmsg->flag = SUCCESS;
    ...
    */
    ...
    sscanf(TypeMd5SizeOffsetStr, "%s%s%ld%ld", fileType,md5Str,&fileSize, &offset);
