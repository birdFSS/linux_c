...
void* threadFun(void *p){
    ...
    while(1)
    {
        ...
        while(!getSuccess){
            ...
            ret = recvCycle(pCur->newFd, &lmsg, 16);//接收大小，标志，文件大小
            if(-1 == ret)
            {
                goto ERROR_DISCONNECT;
            }
            ret = recvCycle(pCur->newFd, lmsg.buf, lmsg.size - 16);
            if(-1 == ret)
            {
                goto ERROR_DISCONNECT;
            }
            ...
            {
            case USERENROLL:
                userEnroll(pCur->newFd,conn, &lmsg);
                break;
            case USERLOGIN:
                userLogin(pCur->newFd, conn, &lmsg);
                break;
