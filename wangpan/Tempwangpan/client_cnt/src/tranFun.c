...
int recordDownloadInfo(char* fileName,char* md5,size_t begin,size_t end);
...
int downloadPart(char* ip,char* port,char* fileName,char* md5,char* passwd, size_t begin, size_t end)
{
    ...
    */
    ...
    if(SUCCESS == lmsg.flag)
    {
        ...
        if(begin + ret < end)
        {
            recordDownloadInfo(fileName,md5,begin+ret,end);
        }
        ret = close(fds[1]);
        ret = close(fds[0]);
        close(fd); 
...
int recordDownloadInfo(char* fileName,char* md5,size_t begin,size_t end)
{
    MYSQL* conn;
    connectMYSQL(&conn);
    char insertInfo[300]="insert into download(fileName,md5,begin,end) values";
    sprintf(insertInfo, "%s('%s','%s',%ld,%ld)", insertInfo, fileName,md5, begin, end);
    char result[100]={0};
    int ret = insertMySQL(conn, insertInfo, result);
    if(ret != 0)
    {
        printf("error insert\n");
        return -1;
