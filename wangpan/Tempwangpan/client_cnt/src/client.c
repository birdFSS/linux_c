...
int checkRecold(char* fileName, char* md5, char* result);
...
int main()
{
    ...
    FILE* fp=fopen("../conf/client.conf", "r");
    ERROR_CHECK(fp, NULL, "confFd");

    int capacity = 10;
    int threadNum = 10;
    char ip[20]={0};
    char port[8]={0};
    fscanf(fp,"%s%s%d%d",ip, port, &capacity, &threadNum);
    fclose(fp);
...
int getsReadyFunc(int socketFd, char* fileName, pFactory_t pf)
{
    ...
    if(FRAG_FLAG == lmsg.flag)
    {
        ...
        int offset = 0;
        int i=0;
        /*查询数据库，如果有，则取出对用值放在这里，如果没有则平均分*/
        size_t begin[2],end[2];
        char beginStr[20]={0};
        char endStr[20]={0};
        char result[600]={0};
        if(!checkRecold(fileName, md5Str, result))
        {
            while(sscanf(result+offset, "%s %s", beginStr,endStr) != -1)
            {
                begin[i]=atol(beginStr);
                end[i]=atol(endStr);
                offset += 2+ strlen(beginStr)+strlen(endStr);
                i++;
            }
        }else{
            begin[0] = 0;
...
int checkRecold(char* fileName, char* md5, char* result)
{
    MYSQL* conn;
    connectMYSQL(&conn);
    char queryInfo[300]="select begin,end from download where fileName=";
    sprintf(queryInfo, "%s'%s' %s'%s'", queryInfo, fileName, "and md5=", md5);
    int ret =queryMySQL(conn, queryInfo,result);
    if(0 != ret)
    {
        return -1;
    }
    char deleteInfo[300]="delete from download where fileName=";
    sprintf(deleteInfo, "%s'%s' %s'%s'", deleteInfo, fileName, "and md5=", md5);
    ret =queryMySQL(conn, deleteInfo,result);
    if(0 != ret)
    {
        printf("删除失败\n");
        return -1;
